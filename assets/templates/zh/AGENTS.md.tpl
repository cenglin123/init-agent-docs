...
- **模式匹配复杂度**：单会话能完成的小任务用直接执行模式；涉及跨模块、预计改动超过 5 个文件、或可能跨会话完成的任务，先在 `docs/plans/active/` 建计划。单会话能完成的小任务不必建计划。
...
<!-- 根据项目实际情况填写。示例：
- 有完整测试套件：后端 `pytest`，前端 `npm test`，改动后必须通过。
...
9. 涉及跨模块、预计改动超过 5 个文件、或可能跨会话完成的任务，先在 `docs/plans/active/` 落一份轻量计划，再开始实施；完成后移到 `docs/plans/completed/`。单会话能完成的小任务不必建计划。

...
对此，converge SKILL 提供了**文档一致性审计模式**（`document-audit`）：定期 spawn 独立 Reviewer 逐条对比文档断言与实际代码/配置，发现 drift → Executor 修正 → 重新验证，形成闭环。建议将文档审计纳入周期性维护（每 5 次 converge 或每月一次），具体协议见 converge SKILL 的 `refs/document-audit.md`。
